...
-----------------------------------------------------------------------------------------
-- LOCAL DISPLAY OBJECTS & BUTTON WIDGETS
-----------------------------------------------------------------------------------------
...
local wordBankLevel1 = { {"Tooth", "Brush"}, {"Basket", "Ball"}, {"Bath", "Room"}, {"Loud", "Speaker"}, {"Cross", "Walk"}, {"Fire", "Fly"}, {"Life", "Time"}, {"Pass", "Port"}, {"Tool", "Box"}, {"Candle", "Stick"} }
...
local function CreateWordListAndSets()
...
function VerifyWordGroup()    
    ...
    if (numWordsCorrect == WORD_LIST_SIZE) then
        ...
        local function nextScene()
            print("enteredscene")
            composer.gotoScene( "beatlevelone" )
        end
...
function scene:create( event )
    ...
    bkg_image = display.newImageRect("Images/Level_1.png", display.contentWidth, display.contentHeight)
